...
if (isset($_POST['created_laundry_type'])) {
    $type = mysqli_real_escape_string($con, $_POST['l_type_desc']);
    $price = mysqli_real_escape_string($con, $_POST['l_type_price']);

    $query = "INSERT INTO l_type (l_type_desc, l_type_price)VALUES ('$type','$price')";

    $query_run = mysqli_query($con, $query);
    if ($query_run) {
        $_SESSION['message'] = "New Laundry Created Successfully";
        header("Location: l_type_create.php");
        exit(0);
    } else {
        $_SESSION['message'] = "New Laundry Type Not Created";
        header("Location: l_type_create.php");
        exit(0);
    }
}
